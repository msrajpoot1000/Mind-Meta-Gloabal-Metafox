...
class MakeSubSubCatBlade extends Command
{
    ...
    $required = in_array('required', $parts); // Check if 'required' is in field string
    $nullable = in_array('nullable', $parts);

    if ($parts[0] === 'input' && count($parts) >= 3) {
        $parsedFields[] = ['type' => 'input', 'inputType' => $parts[1], 'name' => $parts[2], 'required' => $required];
    } elseif ($parts[0] === 'textarea' && count($parts) >= 2) {
        $parsedFields[] = ['type' => 'textarea', 'inputType' => null, 'name' => $parts[1], 'required' => $required];
    } elseif ($parts[0] === 'select' && count($parts) >= 2) {
        $parsedFields[] = ['type' => 'select', 'inputType' => null, 'name' => $parts[1], 'required' => $required];
    }  elseif ($parts[0] === 'ref_id' && $parts[1] === 'belongsTo' && count($parts) >= 3) {
    $modelName = $parts[2];
    $required = in_array('required', $parts);
    $parsedFields[] = ['type' => 'belongsTo', 'inputType' => $modelName, 'name' => 'ref_id', 'required' => $required];
    $belongsToModels[] = $modelName;
}
else {
...
BLADE;
    ...
    {
        $name = $field['name'];
      $requiredMark = !empty($field['required']) ? ' <span class="text-danger">*</span>' : '';
      $label = $this->label($name) . $requiredMark;
...
<div class="row">
    <div class="col-md-6">
        <div class="mb-3">
            ...
            <input type="file" name="$name" id="$name" class="form-control preview-image-input $class" data-preview-id="$previewId" accept="image/*">
            $error
        </div>
    </div>
    <div class="col-md-6 d-flex align-items-center justify-content-center">
        <img id="photo_preview_$name" src="" alt="Preview" style="max-width: 5rem; display: none; border: 1px solid #ccc;">
    </div>
</div>
HTML;
        }
