...
class MakeSubSubBladeEdit extends Command
{
    ...
    public function handle()
    {
        ...
        $topSlug = $this->extractTopSlug($viewName); // e.g., products
        ...
        $formFields = $this->getProgramSubProgramFields($topSlug) . "\n\n" .
...
<div class="row">
    ...
    <div class="col-md-6 d-flex align-items-center justify-content-center">
        <img id="$previewId" src="{{ asset(\$item3->$name) }}" alt="Selected Image" style="max-width: 5rem; border: 1px solid #ccc; padding: 5px;">
    </div>
</div>
HTML;
        }
...
HTML;
   ...
   private function getProgramSubProgramFields(string $topSlug): string
{
    $singular = Str::singular($topSlug);         // e.g., "products" → "product"
    $programLabel = Str::headline($singular);    // e.g., "product" → "Product"
    $subProgramLabel = 'Sub ' . $programLabel;   // e.g., → "Sub Product"

    return <<<HTML
<div class="row">
    <div class="col-md-6">
        <div class="mb-3">
            <label for="programSelect">$programLabel</label>
            <select name="head_ref_id" class="form-control" id="programSelect" required>
                <option value="">-- Select $programLabel --</option>
                @foreach (\$items1 as \$item)
                    <option value="{{ \$item->id }}" {{ \$item->id == \$item1Id ? 'selected' : '' }}>
                        {{ \$item->name }}
                    </option>
                @endforeach
            </select>
    ...
    <div class="col-md-6">
        <div class="mb-3">
            <label for="subProgramSelect">$subProgramLabel</label>
