...
class MakeSubSubBladeEdit extends Command
{
    ...
    public function handle()
    {
        ...
        // Convert RefModel to kebab plural slug, e.g., SubProgram → sub-programs
        $refSlug = $refModel ? Str::kebab(Str::pluralStudly(class_basename($refModel))) : 'sub-items';

        $formFields = $this->getProgramSubProgramFields($refSlug, $topSlug) . "\n\n" .
...
HTML;
    ...
    private function getProgramSubProgramFields(string $refSlug, string $topSlug): string
{
    $singular = Str::singular($topSlug);
    $programLabel = Str::headline($singular);
