...
class MakeSubSubCatBlade extends Command
{
    ...
    public function handle()
    {
        ...
        $subModelSlug = 'sub' . Str::studly(Str::singular($topSlug));
        $subModelVar = Str::camel($subModelSlug);
        $topModelVar = Str::camel(Str::singular($topSlug));

        $refModel = null;
        foreach ($parsedFields as $field) {
            if ($field['type'] === 'belongsTo') {
                $refModel = $field['inputType'];
                break;
            }
        }

        $parentSlug = $refModel && Str::startsWith($refModel, 'Sub')
            ? Str::kebab(Str::replaceFirst('Sub', '', class_basename($refModel)))
            : 'main';
        ...
        File::put($viewPath, $this->generateBladeView($viewName, $parsedFields, $topSlug, $subModelVar, $topModelVar, $parentSlug, $refModel));
    ...
    private function generateBladeView(string $viewName, array $fields, string $topSlug, string $subModelVar, string $topModelVar, string $parentSlug, ?string $refModel): string
    {
        $programLabel = Str::headline(Str::singular($topSlug));
        $subProgramLabel = 'Sub ' . $programLabel;
        $subSlug = Str::kebab(Str::pluralStudly(class_basename($refModel ?? 'SubProgram')));

        $dynamicFieldsHtml = collect($fields)->map(fn($f) => $this->renderField($f))->implode("\n\n");
        $fieldHeaders = collect($fields)->map(fn($f) => '<th>' . $this->label($f['name']) . '</th>')->implode("\n");
        $fieldCells = collect($fields)->map(fn($f) => $this->renderTableCell($f))->implode("\n");

        $relationshipFields = <<<BLADE
<td class="v-center">{{ \$item->{$subModelVar}->{$topModelVar}->name ?? 'N/A' }}</td>
<td class="v-center">{{ \$item->{$subModelVar}->name ?? 'N/A' }}</td>
BLADE;
