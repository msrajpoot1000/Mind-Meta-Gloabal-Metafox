...
class MakeSubCatBlade extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($fields as $field) {
            ...
            if ($parts[0] === 'input') {
                if (count($parts) !== 3) {
                    $this->error("For 'input', use format: input:inputType:name");
                    return;
                }
            ...
            } elseif ($parts[0] === 'ref_id' && $parts[1] === 'belongsTo' && isset($parts[2])) {
                $model = $parts[2];
                $this->belongsToLabel = Str::headline($model); // just "Program"
                $this->belongsToRelation = Str::camel(Str::singular($model)); // "program"
            } else {
                $parsedFields[] = [
                    'type' => $parts[0],
                    'inputType' => null,
    ...
    private function generateBladeView($viewName, $fields): string
    {
        ...
        $dynamicFieldsHtml = collect($fields)->map(fn($field) => $this->renderField($field))->implode("\n\n");
...
HTML;
        ...
        $fieldsHtml = $refIdField . "\n\n" . $dynamicFieldsHtml;

        $tableHeaders = "<th>{$labelText}</th>\n" .
            collect($fields)->map(fn($field) => "<th>" . $this->label($field['name']) . "</th>")->implode("\n");

        $relationCell = <<<HTML
<td class="v-center">{{ \$item->{$this->belongsToRelation}->name ?? 'N/A' }}</td>
HTML;

        $tableCells = $relationCell . "\n";
...
BLADE;
    ...
    private function renderField(array $field): string
    {
        ...
        if ($field['type'] === 'input' && $field['inputType'] === 'file') {
            ...
            $inputId = "{$name}{$this->fileInputCounter}";
...
<div class="row">
    <div class="col-md-6">
        <div class="mb-3">
            ...
            <input type="file" class="form-control preview-image-input $inputClass" data-preview-id="$previewId"  name="$name" id="$name" accept="image/*">
            $errorBlock
        </div>
    </div>
    <div class="col-md-6 d-flex align-items-center justify-content-center">
        <img id="photo_preview_$name" id="photo_preview_$name" alt="Selected Image" style="max-width: 5rem; display: none; border: 1px solid #ccc; padding: 5px;">
    </div>
</div>
HTML;
        }
...
HTML;
    ...
    private function getCategoryLabel(string $viewName): string
    {
