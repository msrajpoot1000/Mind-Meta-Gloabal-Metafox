...
class MakeSubSubCatBlade extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($fields as $field) {
            ...
            if ($parts[0] === 'input' && count($parts) === 3) {
                ...
                $parsedFields[] = ['type' => $parts[0], 'inputType' => null, 'name' => $parts[1]];
            } else {
        ...
        $topSlug = $this->extractTopSlug($viewName);                         // products
        $subModelSlug = 'sub' . Str::studly(Str::singular($topSlug));       // SubProduct
        $subModelVar = Str::camel($subModelSlug);                           // subProduct
        $topModelVar = Str::camel(Str::singular($topSlug));                 // product
        ...
        File::put($viewPath, $this->generateBladeView($viewName, $parsedFields, $topSlug, $subModelVar, $topModelVar));
    ...
    private function generateBladeView(string $viewName, array $fields, string $topSlug, string $subModelVar, string $topModelVar): string
    {
        $programLabel = Str::headline(Str::singular($topSlug)); // e.g., Product
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        document.getElementById('programSelect').addEventListener('change', function () {
            ...
            if (programId) {
              fetch(`/get-sub-{$topSlug}/\${programId}`)
