...
class MakeSubCatBlade extends Command
{
    ...
    {
        $labelText = $this->getCategoryLabel($viewName);
...
HTML;
        ...
        $relationName = Str::singular(Str::camel(last(explode('-', $viewName))));
        $tableCells = "<td class=\"v-center\">{{ \$item->{$relationName}->name ?? 'N/A' }}</td>\n";
...
HTML;
        ...
        if ($field['type'] === 'textarea') {
            ...
            $textareaId = "textarea{$this->textareaCounter}";
            return <<<HTML
<div class="mb-3">
