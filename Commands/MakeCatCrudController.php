...
class MakeCatCrudController extends Command
{
    ...
    $fields[] = [
        ...
        'default' => collect($parts)->filter(fn($p) => Str::startsWith($p, 'default='))->first(),
    ];
}
...
// Handle default
if ($field['default']) {
    $defaultValue = explode('=', $field['default'])[1];
 ...
 protected function getControllerStub($fields, $modelName, $snakeSingular, $snakePlural, $routeSlug)
{
    ...
    foreach ($fields as $field) {
        ...
        if ($type === 'image') {
            ...
            // ✅ store() uses full image validation
            $ruleParts = ['image', 'mimes:jpg,jpeg,png', 'max:2048'];
            $ruleParts[] = $field['required'] ? 'required' : 'nullable';
            $rules[] = "'{$name}' => '" . implode('|', $ruleParts) . "'";

            // ✅ update() uses status validation
            $statusField = 'status_' . $name;
            $updateRules[] = $field['required']
                ? "'{$statusField}' => 'required|in:1'"
                : "'{$statusField}' => 'nullable|in:0,1'";
        } else {
            $nonImageFields[] = $name;
            ...
            $ruleString = "'{$name}' => '" . implode('|', $ruleParts) . "'";
            $rules[] = $ruleString;
...
PHP;
    ...
    $onlyArray = implode("', '", $nonImageFields);
