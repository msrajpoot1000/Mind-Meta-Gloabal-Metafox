...
$updateRules = [];
    ...
    if ($type === 'image') {
        ...
        // Store validation
        $storeRule = [];
        $storeRule[] = $field['required'] ? 'required' : 'nullable';
        $storeRule[] = 'image';
        $storeRule[] = 'mimes:jpg,jpeg,png';
        $storeRule[] = 'max:2048';
        $storeRules[] = "'{$name}' => '" . implode('|', $storeRule) . "'";

        // Update validation (use status_photo format)
       $statusField = 'status_' . $name;
if ($field['required']) {
    $updateRules[] = "'{$statusField}' => 'required|in:1'";
} else {
    $updateRules[] = "'{$statusField}' => 'nullable|in:0,1'";
}
    ...
    $joined = implode('|', $ruleParts);
    $storeRules[] = "'{$name}' => '{$joined}'";
    $updateRules[] = "'{$name}' => '{$joined}'";
}
